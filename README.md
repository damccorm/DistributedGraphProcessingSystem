...
* vertex.incoming_messages - list of incoming messages from the previous round
...
The compute function should have the form of "lambda vertex, input_value, send_message_to_vertex: compute(vertex, input_value, send_message_to_vertex)", where vertex is the Vertex object the function is manipulating, input_value is the result of the aggregation, and send_message_to_vertex is an anonymous function that can be used to send a message along any edge, with the form send_message_to_vertex(sending_vertex, receiving_vertex_number, contents).
